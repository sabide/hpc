!**********************************************************************
!   poisson_nonblocking.f90 - Solving the Poisson's equation discretized on the [0,1]x[0,1 domain
...
PROGRAM poisson
  ...
  DO WHILE ((.NOT. convergence) .AND. (it < it_max))
    ...
    CALL start_communications(u)

    !Computation of u at the n+1 iteration
    CALL computation(u, u_new, sx+1, ex-1, sy+1, ey-1)

    CALL end_communications

    ! North
    CALL computation(u, u_new, sx, sx, sy, ey)
    ! South
    CALL computation(u, u_new, ex, ex, sy, ey)
    ! West
    CALL computation(u, u_new, sx, ex, sy, sy)
    ! East
    CALL computation(u, u_new, sx, ex, ey, ey)
