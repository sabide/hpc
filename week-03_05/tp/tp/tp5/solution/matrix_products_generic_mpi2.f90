program matrix_products
  ...
  integer                             :: rank, Nprocs, N, NL, code, k
  ...
  integer                             :: NLmax, remainder, type_slice1, type_slice2
  integer                             :: krank
  integer, allocatable, dimension(:)  :: Ndist
  integer, allocatable, dimension(:)  :: nbrS, dispS, typeS
  integer, allocatable, dimension(:)  :: nbrR, dispR, typeR
  ...
  call MPI_ALLGATHER(NL,1,MPI_INTEGER,Ndist,1,MPI_INTEGER,MPI_COMM_WORLD,code)
  ...
  call MPI_TYPE_VECTOR(N, NLmax, N, MPI_REAL, type_slice1, code)
  call MPI_TYPE_COMMIT(type_slice1, code)
  call MPI_TYPE_VECTOR(N, NLmax-1, N, MPI_REAL, type_slice2, code)
  call MPI_TYPE_COMMIT(type_slice2, code)
  do k=1, Nprocs
  ...
  dispR(:) = 0
  typeR(:) = MPI_REAL
  call MPI_ALLTOALLW(A, nbrS, dispS, typeS, AL, nbrR, dispR, typeR, MPI_COMM_WORLD, code)
  ...
  call MPI_SCATTERV(B, nbrS, dispS, MPI_REAL, BL, N*NL, MPI_REAL, 0, MPI_COMM_WORLD, code)
...
!  end do
  ...
  call MPI_GATHERV(CL, NL*N, MPI_REAL, C, nbrS, dispS, MPI_REAL, 0, MPI_COMM_WORLD, code)
  ...
  call MPI_TYPE_FREE(type_slice1,code)
  call MPI_TYPE_FREE(type_slice2,code)
